...
| **Рідна основа macOS** | Swift + SwiftNIO + SwiftUI/AppKit | Нативний застосунок macOS із закритим кодом | Кросплатформенний застосунок із закритим кодом |
| **Локальне захоплення** | Локальний проксі-сервер, сертифікати, допоміжна програма та дані захоплення залишаються на вашому Mac | Настільний проксі-додаток | Настільний проксі-додаток |
| **Робочий процес налаштування розробника** | Вбудований центр налаштування розробника для середовища виконання, клієнтів, пристроїв, фреймворків і середовищ | Вбудоване автоматичне налаштування плюс посібники для платформ і середовищ виконання | Посібники з налаштування для конкретної платформи |
| **Зовнішній проксі + маршрутизація PAC** | Вихідний проксі HTTP/HTTPS, автоматична конфігурація PAC і правила обходу | Комерційний вихідний проксі та підтримка PAC | Комерційна конфігурація вихідного проксі |
| **Інтеграція MCP** | [Вбудований локальний MCP](docs/features/mcp.mdx): 10 інструментів лише для читання для трафіку, статусу, сертифікатів, перевірки правил та експорту cURL; автентифікація за токеном; redaction увімкнено за замовчуванням | Вбудований локальний MCP: перевірка трафіку та інструменти правил, сесій, сертифікатів, налаштування й керування застосунком; лише localhost; автентифікація за токеном для кожної сесії; redaction конфіденційних даних | Первинної інтеграції MCP не знайдено в [офіційній документації](https://www.charlesproxy.com/documentation/), перевіреній 2026-08-13 |
| **Нативний AI Assistant** | Вбудований для аналізу трафіку за вибраним запитом і кількома запитами всередині Rockxy | Невідомо | Невідомо |
| **Відкритий шлях внеску** | Публічний вихідний код, issues, обговорення, дорожня карта та PR | Публічний трекер issue; вихідний код застосунку та релізи контролюються постачальником | Документація й підтримка постачальника; вихідний код застосунку та релізи контролюються постачальником |

Наведені вище можливості конкурентів було перевірено за офіційною документацією продукту 2026-08-13 і вони можуть змінитися після публікації.

На дорожній карті: глибші protocol-aware rules, безпечніші redacted evidence bundles, сильніші workflow replay і comparison, ширші посібники Developer Setup та подальше дослідження HTTP/2 і HTTP/3.
