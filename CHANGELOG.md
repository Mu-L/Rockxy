...
- Fixed a crash when opening Developer Setup: its window could enter an AppKit "Update Constraints in Window" layout loop from custom toolbar glass, a centered toolbar item, self-measuring rows, and a whole-split-view minimum width once the readiness inspector opened. The section switcher now lives in a native segmented header, the inspector opens without pushing the sidebar off-screen, and status text no longer re-wraps against the column width.
- Made proxied WebSocket connections appear in the request list as soon as the upgrade completes, with the `101 Switching Protocols` handshake headers, live frame counts, and a Closed state plus connection lifetime when the socket ends, instead of only surfacing after the connection closed and then staying "Active" forever.
- Stopped the sidebar Apps list from showing the same client twice with split counts (for example `openai-python` twice) when process attribution resolved after the first requests; app rows are now grouped by the label the Client column shows, and unattributed rows read "Unknown" instead of a bare "?" badge.
- Fixed AI inspection for common SDK traffic: finish reasons (`stop`, `length`, `tool_calls`, `end_turn`, `tool_use`) are surfaced, Anthropic streamed usage is merged across `message_start` and `message_delta` including cache reads, streamed tool-call arguments are reassembled for OpenAI Chat, Responses, and Anthropic streams, the requested versus served model is shown, and the usage bar no longer double-counts cached tokens. Gemini `generateContent` and Ollama `/api/chat` NDJSON traffic are now recognized.
- Replaced the AI inspector's decorative stream bars with per-event payload sizes, added readable event names and text previews for unnamed SSE chunks, an Assembled Output section reassembled from stream deltas, provider error bodies plus `Retry-After` and rate-limit headers for 429/503 responses, and the provider request id; labels no longer wrap mid-word in narrow inspectors.
- Made the Context Dock's "slower than this host" insight switch to a multiplier (for example "About 180× the session median") once a request runs past 3× the median instead of printing unreadable four- and five-digit percentages, and counted WebSocket frame bytes in Transferred.
- Unified in-app wording on "HTTPS Decryption" where toasts, Map Local guidance, and the AI session hint still said "SSL Proxying".
- Stopped Developer Setup from reporting "Local probe unavailable" for manual-only device targets such as iOS Simulator after a readiness refresh; those targets now keep their manual-validation guidance.
- Made Repeat add the replayed exchange to the request list as its own row (attributed to Rockxy, selected on arrival) so a re-sent chat completion or retried call can be inspected, diffed, and exported like any capture instead of disappearing behind a "Replay completed" toast.
- Fixed HAR export writing Rockxy's internal category (`json`, `text`, `unknown`) as `mimeType` instead of the captured `Content-Type`, so Chrome DevTools, Charles, and Proxyman can render imported bodies; streamed `text/event-stream` and `application/x-ndjson` responses are exported as readable text rather than base64, and replayed or restored rows without a timing breakdown now export their measured duration instead of `0`.
- Preserved the measured lifetime of WebSocket connections (and replayed rows) in saved sessions, so a reopened session no longer shows a blank duration for sockets that were closed.
- Stopped labeling an ordinary `/v1/messages` chat or inbox endpoint on a non-Anthropic host as "AI API · Anthropic"; off `api.anthropic.com` that route now needs Messages API shape (the `anthropic-version` header, a `model` field, or a `stop_reason`) before it is treated as AI traffic.
- Added a `model:` search token alongside `provider:` so AI traffic can be narrowed to a requested model (for example `model:gpt-4o` or `provider:anthropic model:sonnet`), and documented the smart tokens in the search field's tooltip.
...
- Stopped Rockxy's own responses (Repeat, Compose, nearby transfers, Babylon capture, and Map Local drafts) from showing CFNetwork wording such as `200 no error`; they now carry the standard HTTP reason phrase (`200 OK`, `429 Too Many Requests`) like proxy-captured rows.
- Fixed the WebSocket inspector reporting `Duration 0s` for any connection shorter than a second; sub-second and sub-minute lifetimes are now shown with the same precision as the Context Dock.
- Fixed rule windows printing "1 rules"; Block List, Allow List, Map Local, Map Remote, Modify Headers, and Network Conditions counts now use proper singular and plural forms.
- Gave the Block List the same empty state as the other rule windows, including a distinct "no matching rules" message while a search is active instead of always inviting a new entry.
- Extended the singular/plural fix across the rest of the app, so counts read "1 request", "1 error", "1 item waiting", "1 muted source", "1 plugin", "1 definition", and so on in the status bar, breakpoint queue, sidebar badges, inspectors, rule and script windows, settings lists, and import/export/Gist toasts instead of "1 requests". Publishing to Gist now also carries a translatable count instead of an English-only suffix.
...
- Made the AI Assistant's traffic-scope menu and its VoiceOver labels read "1 request" instead of "1 requests", and rebuilt the scope title as one translatable sentence so the attached share ("1 of 12 requests") no longer assembles an English word order in other languages.
- Translated the two rule-storage failure alerts, which stayed in English because their messages were never extracted into the localization catalog.
- Made every surface report the same duration for the same exchange. The Context Dock details table, the AI Assistant's evidence, and the WebSocket inspector each formatted durations on their own, so a 90-second stream read `90.00 s` in the dock but `1m 30s` in the request list, a sub-millisecond request read `0 ms` instead of `412 µs`, and a 119.6-second connection could print `1m 60s`. Durations are now rounded before their unit is chosen, so a value never spills past the unit it is shown in.
...
- Made counted badges and table values read the same way as the sentences beside them. Token totals, stream event counts, WebSocket frame counts, request-header counts, sidebar and toolbar badges, and the Context Dock's selection summary were written straight into the interface without a number formatter, so they lost the viewer's digit grouping: the captured-value picker showed a badge of "1234" directly above its own tooltip's "1,234 captured requests", and a context window read "128000" in the AI inspector where the Review Data sheet said "128.000".
- Made strings that Rockxy draws through SwiftUI follow the app's own Language setting. Cookie and header inspector columns, the GraphQL and Timing tabs, the log stream and plugin empty states, the scripting editor's labels, and the advanced filter bar's shortcut hints were resolved against macOS's language instead of Rockxy's, so choosing 简体中文 inside Rockxy left them in English with no way to fix it.
- Fixed the nearby-transfer confirmation alert, which was never translated and read "contains 1 requests" for a single-request session.
- Made the JSON tree's query status and collapsed-item counts translatable, and the diff summary's added/removed counts grouped.
- Fixed port numbers being printed with digit separators. The status bar's system-proxy override text read "Port=9,090", Developer Setup's proxy step told a mobile developer Rockxy was "listening on 127.0.0.1:9,090", the MCP settings row said "Running on port 9,797", and an upstream-proxy test result reported "host:8,443". Keychain status codes were at the same risk and are now kept numeric too.
- Translated the privileged helper, keychain, and certificate failure messages, which reached the certificate wizard's "Installation failed" panel, Settings ▸ Advanced ▸ Privileged Helper Tool's "Last Error" box, and the Welcome window in English only — including the ones that tell you what to do next, such as unlocking the login keychain or updating the helper.
- Made decimal numbers follow the viewer's region. Durations, waterfall and diff timings, the AI inspector's score, the network-condition packet-loss figure, and the import size limits were written with a fixed English decimal point, while byte counts followed the region — so in a comma-decimal region such as Vietnam or Germany the Context Dock read a duration of "5.64 s" directly beside its own footer's "1,5 MB total" for the same session. Percentages now take the region's own sign placement too ("45 %" in German and French), and a duration that rounds up to 1000 µs is shown as "1 ms".
- Made the Focus navigator's Noise Control section show what is muted. The section drew its header and nothing else, so the only trace of a muted host or path lived inside the Noise Control sheet and a request that was hidden looked like a request that was never made; it now lists each muted source with the number of captured requests it is hiding, and offers Unmute from its context menu.
- Fixed the Focus navigator's two sections staying blank after their contents changed — muting a source while Focus mode was on screen emptied the section until the navigator was switched away and back.
- Localized the nearby-transfer errors shown while pairing or importing a session from another Mac, which were English-only regardless of the selected language.
- Showed an Export Failed alert when saving a request or response body to disk fails, instead of failing silently.
...
- Fixed Pro license settings so Mac activation counts and plan names reflect the correct Mac-specific activation limit instead of a combined provider limit.
...
- Initial open-source release of Rockxy
