...
struct AIAssistantDockView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text(String(
                localized: "This removes the conversation from this workspace history.",
    ...
    private var selectedScopeTitle: String {
        AssistantScopeSummary.selectedScopeTitle(
            attached: selectedContextCount,
            selected: selectedTransactions.count
        )
    }
    ...
    private var assistantHeader: some View {
        HStack(spacing: 8) {
            ...
            .popover(isPresented: $isConversationSwitcherPresented, arrowEdge: .top) {
                AssistantConversationHistoryView(
                    coordinator: coordinator,
                    isPresented: $isConversationSwitcherPresented,
                    conversationBeingRenamed: $conversationBeingRenamed,
                    conversationRenameDraft: $conversationRenameDraft,
                    conversationPendingDeletion: $conversationPendingDeletion,
                    onSelect: { isComposerFocused = true }
                )
            }
    ...
    @ViewBuilder private var attachedContextHeader: some View {
        if let transaction = primaryTransaction {
            HStack(spacing: 8) {
                ...
                Menu {
                    ...
                    Button {
                        ...
                        Label(
                            String(
                                localized: "Include Related Requests (+\(relatedTransactionCount))",
                                bundle: RockxyLocalization.bundle
                            ),
                    ...
                    Label(CountFormatter.format(contextTransactions.count), systemImage: "paperclip")
                ...
                .accessibilityLabel(
                    AssistantScopeSummary.scopeAccessibilityLabel(attached: contextTransactions.count)
                )
            }
            .padding(.horizontal, 10)
            ...
            .background(Color.clear)
            .accessibilityElement(children: .contain)
            .accessibilityLabel(AssistantScopeSummary.attachedTrafficAccessibilityLabel(
                summary: requestSummary(for: transaction),
                attached: contextTransactions.count
            ))
        } else {
            HStack(spacing: 8) {
    ...
    private var noSelectionEmptyState: some View {
        VStack(spacing: 6) {
            Text(String(localized: "Investigate captured traffic", bundle: RockxyLocalization.bundle))
                .font(assistantFont(appMetrics.primaryFontSize, weight: .semibold))
            Text(String(
                localized: "Select a request to investigate, or type a question below.",
                bundle: RockxyLocalization.bundle
            ))
            .font(assistantFont(appMetrics.secondaryFontSize))
            .foregroundStyle(.secondary)
    ...
    @ViewBuilder private var activeAssistantTurn: some View {
        ...
        case let .investigating(_, recipe):
            workEvent(
                ...
                detail: String(
                    localized: "Gathering evidence for \(recipe.title.lowercased()).",
                    bundle: RockxyLocalization.bundle
                ),
                ...
                if text.isEmpty {
                    Text(
                        executionLocation.isLocal
                            ? String(
                                localized: "The local model is reading the reviewed request context.",
                                bundle: RockxyLocalization.bundle
                            )
                            : String(
                                localized: "The configured provider is processing the reviewed request context.",
                                bundle: RockxyLocalization.bundle
                            )
                    )
    ...
    private var promptComposer: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                ...
                .fixedSize()
                .help(String(
                    localized: "Choose local analysis or the app-wide AI model",
                    bundle: RockxyLocalization.bundle
                ))
                ...
                .accessibilityLabel(String(
                    localized: "Read-only Assistant privacy details",
                    bundle: RockxyLocalization.bundle
                ))
        ...
        case .idle:
            if resultIsInTranscript(result) {
                ...
                assistantBubble {
                    ...
                    Text(String(
                        localized: "Preparing the exact request for Review Data.",
                        bundle: RockxyLocalization.bundle
                    ))
                    .font(assistantFont(appMetrics.metadataFontSize))
                    .foregroundStyle(.secondary)
                ...
                assistantBubble {
                    ...
                    Text(String(
                        localized: "Confirm the redacted traffic and conversation before the model runs.",
                        bundle: RockxyLocalization.bundle
                    ))
                    .font(assistantFont(appMetrics.metadataFontSize))
                    .foregroundStyle(.secondary)
                ...
                assistantBubble {
                    AssistantMarkdownText(
                        source: message.text.isEmpty
                            ? String(
                                localized: "The model completed without returning text.",
                                bundle: RockxyLocalization.bundle
                            )
                            : message.text
                    )
    ...
    private func blockedToolCallWarning(_ count: Int) -> some View {
        Text(String(AttributedString(
            localized: "Rockxy blocked ^[\(count) model action request](inflect: true).",
            bundle: RockxyLocalization.bundle,
        ...
        -> some View
    {
        Menu {
            ...
            if let usage {
                Divider()
                Button(String(
                    localized: "\(usage.inputTokens) input · \(usage.outputTokens) output",
                    bundle: RockxyLocalization.bundle
                )) {}
                    .disabled(true)
            }
        ...
        .accessibilityLabel(String(
            localized: "Model details: \(provider), \(model), \(endpointHost)",
            bundle: RockxyLocalization.bundle
        ))
    }
