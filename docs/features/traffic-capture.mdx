...
| Pause/Resume Recording | `Option + Cmd + R` | Pause/resume recording without stopping the proxy |
...
- **Copy as code** — right-click any request > **Copy as** > **Swift (URLSession)**, **Python (requests)**, **JavaScript (fetch)**, or **Go (net/http)** for a snippet that reproduces the request in that language. Transport-managed headers (`Host`, `Content-Length`, `Proxy-*`) are left out because each client library derives them.
