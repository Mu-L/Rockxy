...
struct DeveloperSetupViewModelTests {
    ...
    func sourceListFilterFindsAutomationTargets() {
        ...
        let sections = viewModel.filteredTargetSections
        let matchingIDs = sections.flatMap(\.targets).map(\.id)
    ...
    @Test("Automatic setup availability follows the selected target")
    func automaticSupportFollowsTargetSupport() async {
        let viewModel = DeveloperSetupViewModel(coordinator: MainContentCoordinator())
    ...
    func inspectorSetupActionsKeepManualAvailableForManualTargets() async {
        ...
        #expect(actions.automaticCaption.contains("Manual Setup"))
        #expect(actions.automaticCaption.localizedCaseInsensitiveContains("not shipped") == false)
    }
    ...
    func flutterHybridWorkflow() async {
        ...
        #expect(viewModel.toolbarCopyEnabled == false)
        viewModel.selectedTab = .snippets
    ...
    func validationMatcher() throws {
        ...
        let oldRequest = HTTPTransaction(
        ...
        let newRequest = HTTPTransaction(
            request: HTTPRequestData(
                method: "GET",
                url: validationURL,
                httpVersion: "HTTP/1.1",
    ...
    func validationInstructionClarifiesGenericCaptureCheck() throws {
        ...
        #expect(validation.instruction
            .contains("specific app, process, device, simulator, emulator, or runtime") == true)
    }
    ...
    @Test("Terminal verification outcomes recreate the validation probe")
    func terminalVerificationOutcomesRecreateProbe() async {
        for terminalState in [
            ...
            let viewModel = DeveloperSetupViewModel(coordinator: MainContentCoordinator())
            viewModel.snapshot.verificationState = terminalState
            await viewModel.stopValidationProbe()

            await viewModel.refreshSnapshot()

            #expect(viewModel.snapshot.verificationState == terminalState)
            #expect(viewModel.hasValidationProbeForSelectedTarget)
            await viewModel.stopValidationProbe()
        }
    }
