...
struct ProtocolDetectorHandlerTests {
    ...
    func rejectsNonHTTPPrefaces() {
        let clientHello =
            ByteBuffer(bytes: [0x16, 0x03, 0x01, 0x02, 0x00, 0x01, 0x00, 0x01, 0xFC, 0x03, 0x03] + [UInt8](
                repeating: 0,
                count: 32
            ))
        ...
        #expect(!ProtocolDetectorHandler
            .looksLikePlainHTTPRequest(Self.buffer("SSH-2.0-OpenSSH_9.0\r\nmore-bytes-here")))
